...
class Teachers extends CI_Controller{
	 ...
	 *@param $start - Starting of teachers table's index to get query
	 *
	 */
    function index($start=0){
		$limit = 10;
        $data['teachers'] = $this->Teachers_model->get_limit_teachers($limit,$start);
		//pagination
		$config['base_url'] = site_url('admin/teachers/index');
		$config['total_rows'] = $this->Teachers_model->get_count_teachers();
		$config['per_page'] = 10;
        ...
        $data['_view'] = 'admin/teachers/index';
...
'status' => html_escape($this->input->post('status')),
						...
						$config['upload_path']          = "./public/uploads/images/teachers";
							  ...
							  if(strlen($_FILES['file_picture']['name'])>0 && $_FILES['file_picture']['size']>0)
								{
									...
									{
										$file_picture = "uploads/images/teachers/".$_FILES['file_picture']['name'];
		...
		//update		
        if(isset($id) && $id>0){
			$data['teachers'] = $this->Teachers_model->get_teachers($id);
            if(isset($_POST) && count($_POST) > 0){   
                $this->Teachers_model->update_teachers($id,$params);
				$this->session->set_flashdata('msg','Teachers has been updated successfully');
                redirect('admin/teachers/index');
            }else{
                $data['_view'] = 'admin/teachers/form';
                $this->load->view('layouts/admin/body',$data);
            }
        } //save
		else{
			if(isset($_POST) && count($_POST) > 0){   
                $teachers_id = $this->Teachers_model->add_teachers($params);
				$this->session->set_flashdata('msg','Teachers has been saved successfully');
                redirect('admin/teachers/index');
            }else{  
			    $data['teachers'] = $this->Teachers_model->get_teachers(0);
                $data['_view'] = 'admin/teachers/form';
                $this->load->view('layouts/admin/body',$data);
            }
		}
	...
	function details($id){
        $data['teachers'] = $this->Teachers_model->get_teachers($id);
		$data['id'] = $id;
        $data['_view'] = 'admin/teachers/details';
        $this->load->view('layouts/admin/body',$data);
	}
     ...
     */
    function remove($id){
        $teachers = $this->Teachers_model->get_teachers($id);

        // check if the teachers exists before trying to delete it
        if(isset($teachers['id'])){
            $this->Teachers_model->delete_teachers($id);
			$this->session->set_flashdata('msg','Teachers has been deleted successfully');
            redirect('admin/teachers/index');
        }
        else
            show_error('The teachers you are trying to delete does not exist.');
    }
...
$this->db->or_like('status', $key, 'both');
		...
		//pagination
		$config['base_url'] = site_url('admin/teachers/search');
		$this->db->reset_query();		
		$this->db->like('id', $key, 'both');
$this->db->or_like('email', $key, 'both');
$this->db->or_like('password', $key, 'both');
...
$this->db->or_like('status', $key, 'both');
		...
		$config['total_rows'] = $this->db->from("teachers")->count_all_results();
		$db_error = $this->db->error();
		if (!empty($db_error['code'])){
			echo 'Database error! Error Code [' . $db_error['code'] . '] Error: ' . $db_error['message'];
			exit;
	...
	function export($export_type='CSV'){
	  if($export_type=='CSV'){	
		   // file name 
		   $filename = 'teachers_'.date('Ymd').'.csv'; 
		   header("Content-Description: File Transfer"); 
		   header("Content-Disposition: attachment; filename=$filename"); 
		   header("Content-Type: application/csv; ");
		   // get data 
		   $this->db->order_by('id', 'desc');
		   $teachersData = $this->Teachers_model->get_all_teachers();
		   // file creation 
		   ...
		   fputcsv($file, $header);
		   foreach ($teachersData as $key=>$line){ 
			 fputcsv($file,$line); 
		   }
		   fclose($file); 
		   exit; 
		   ...
		   // get the HTML
			ob_start();
			include(APPPATH.'views/admin/teachers/print_template.php');
			$html = ob_get_clean();
